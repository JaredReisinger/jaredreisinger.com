...
(function($) {
  ...
  jQuery.fn.scrollex = function(userOptions) {
    ...
    // Multiple elements?
    if (this.length > 1) {
      for (var i = 0; i < this.length; i++) $(this[i]).scrollex(userOptions);
    ...
    handler = function(vTop) {
      ...
      eBottom =
        offset.top + eHeight - resolve(this.options.bottom, eHeight, vHeight);
  ...
  jQuery.fn.unscrollex = function() {
    ...
    // Multiple elements?
    if (this.length > 1) {
      for (var i = 0; i < this.length; i++) $(this[i]).unscrollex();
